...
export const useStagedList = (existingRows, { idField = 'id' } = {}) => {
  ...
  // Patches a not-yet-persisted `creates` entry in place — distinct from
  // `edit`, which queues a patch against an already-persisted row.
  const editCreate = useCallback((tempId, patch) => {
    setCreates(prev => prev.map(c => c.tempId === tempId ? { ...c, data: { ...c.data, ...patch } } : c));
  }, []);
  ...
  return { items, isDirty, add, edit, editCreate, remove, commit };
};
